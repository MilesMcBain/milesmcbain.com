...
![Jump to source, loadd targets at cursor, run code.](/content/images/2020/04/drake_cache2.gif)
...
![fnmate-1](/content/images/2020/04/fnmate-1.gif)
...
Where `drake::readd` pulls `coverage_map` from the cache into the rendering environment. It also signifies to `drake` that `report` has a dependency on `coverage_map`.^[Yes. Incredibly `drake` parses the code inside your RMarkdown and registers them as dependencies of the ]
