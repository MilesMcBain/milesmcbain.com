...
It's a kind of curse of the data analyst: You're desperate to prove you can use
these much-hyped skills to add value. But as soon as you do, you're stuck. Your
value-add becomes business as usual. And good luck to you if you think you're
going to pass up business as usual to investigate more interesting data - that
would be a value-subtract. And those make the
well-dressed-iPad-with-soft-keyboard-under-arm people very unhappy.
...
you all get bored and quit.

The sort of mind that will tolerate the guts of computers, and the various ways
they can torture data, does so out of a kind of insatiable curiosity. That
curiosity can be harnessed to great productive effect. But remove the
opportunity to be curious, and exploratory with data problems, and morale
suffers.
...
making two pretty big assumptions in this discussion:
  ...
  1. Your work is written in code e.g. R, Python, Julia, Rust etc. 
   ...
   2. You use a technology like [Quarto], or [Rmarkdown], or [Shiny], such that
...
first this may feel pretty good. Almonds like cheating. If you're clever you
won't tell your boss / clients how much time you just saved. And not because
...
productivity gains you got from the copy-paste in the first place. [^The interest on techincal debt is debited at the least opportune time.]
...
synhcronised copy-pastes in source code and document files.]
...
might even employ metaprogramming techniques to conditionally change the way
your outputs look.
...
for this occasion:
    ...
    Any sufficiently complicated plain text configuration scheme contains an ad hoc, informally-specified, bug-ridden, slow implementation of half of Common Lisp.
...
have to be updated in a syncronised fashion, and that sounds like an awkward
...
workflow with lots of duplicated code, into style based on functional
composition. 

Massive functions get written with perhaps a dozen arguments, containing
hundreds of lines of code, that are really not much more than:

```
wrangle_data.script` rebadged as `wrangle_data(x, y, z, cubits, flotsits, n_widgets, calibration_factor, tolerance, initial_state, start_date, end_date, output_path)
```
...
Such a function is pretty much untestable due to the combinatoric explosion of
...
the screams of matrices being ground to a fine mist of bits through the conical
...
both analysis projects and monolithic packages into modular 'universe' of
...
macros[^ Okay in R, they're 'target factories' that depend on R's special flavour of metaprogramming.]
...
wrap up API endpoints or SQL queries to provide easy commonly used source data.
...
up in your searches.[^Okay probably you are because you'll have that one colleague that will cling to it.]
...
practices like 'dogfooding'[^that is 'eating your own dogfood' or using your own tools to understand their strengths and weeknesses], seeking peer feedback
...
from scratch every time, typing out the same boilerplate to load your tools,
and ingest the data can feel like a bit of a waste.
...
but leaves the user to flesh out the (hopefully) interesting
parts.
